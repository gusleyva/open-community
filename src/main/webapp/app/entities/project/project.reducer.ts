...
export const getSearchEntities: ICrudSearchAction<IProject> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_PROJECTS,
  payload: axios.get<IProject>(`${apiSearchUrl}?query=${query}${sort ? `&page=${page}&size=${size}&sort=${sort}` : ''}`),
});
